...
Например инвентарь кластера из 50 инстансов может легко перевалить за тысячу строк.
...
```shell
curl -sLO https://binary.picodata.io/repository/raw/genin/bin/genin-0.3.7-x86_64-musl.tar.gz
tar -xvf genin-0.3.7-x86_64-musl.tar.gz ; sudo install genin /usr/local/bin/
```
...
```shell
curl -sLO https://binary.picodata.io/repository/raw/genin/osx/genin-0.3.7-x86_64-macosx.tar.gz
...
```shell
curl.exe -sLO https://binary.picodata.io/repository/raw/genin/win/genin-0.3.7-win64.zip
unzip.exe genin-0.3.7-win64.zip -d %HOME%/.cargo/bin/
```
...
genin --version
```
Если вы видите сообщение `genin 0.3.7` значит установка прошла успешно.
...
```yaml
---
# Список инстансов которые должны быть в кластере представлен в виде массива
instances:
  # Каждый инстанс это объект (map) в массиве
  - name: router              # (обязательное поле) имя инстанса
    type: router              # (обязательное поле) тип инстанса (storage, router, custom, dummy, replica)
    count: 1                  # (опционально) как много мастеров нам нужно, по умолчанию 1
    replicas: 0               # (опционально) количество реплик, по умолчанию 1 для типа storage, и 0 для всех остальных типов
    weight: 10                # (опционально) вес инстанса
    roles:                    # (опционально) список ролей в виде массива   
      - router
      - api
      - failover-coordinator
    config:                   # (опционально) конфиг с параметрами для инстанса
      instance_name: router   # любые значения которые поддерживает ansible cartridge в произвольном порядке

  # Далее будут остальный типы инстансов
  - name: storage
    type: storage
    count: 2
    replicas: 2
...
до 10. Изменим количество реплик у хранилищ (storage) до 1, и добавим кастомный инстанс
