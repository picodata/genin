...
```shell
curl -sLO https://binary.picodata.io/repository/raw/genin/osx/genin-0.5.6-x86_64-macosx.tar.gz
...
```shell
curl.exe -sLO https://binary.picodata.io/repository/raw/genin/win/genin-0.5.6-win64.zip
unzip.exe genin-0.5.6-win64.zip -d %HOME%/.cargo/bin/
```
...
genin --version
```
Если вы видите сообщение `genin 0.5.6` значит установка прошла успешно.
