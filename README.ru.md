...
Аргумент `--export-state` может быть полезен для того что бы экспортировать [стейт](#Genin-state)
...
эквивалентны. Например `api-1` и `api-1-1` буду считься репликами одного репликасета.
С добавление опции без точного совпадения имени реплики никогда не будут объеденяться 
в репликасет.
...
генерировать инвентари на основе двух разных конфигураций кластера. У нее есть несколько
существенных отличий от команды `build`. Например если взять исходную конфигурацию с 4 
серверами в 2 датацентрах, увеличить количество серверов вдвое, а так же добавить 
...
Поэтому специально для случаев **апгрейда** сущестует команда `genin upgrade`.
> **Note:** `upgrade` это последовательное изменение кластера основыванное на 
> исходной конфигурации `--old` и целевой конфигурации `--new`. При которой 
...
старые получат только те параметры что можно изменить не сломав кластер.
...
`genin build`. То есть мы сначала распределяем конфигурацию переданную в `--old` в потом 
...
Специально что бы избежать этого в `genin` было добавлено сохранение метаданных и дерева
...
Имена файлов `state` могут быть двух типов. Первый это `latest` стейт сгенерированный при 
...
Для просмотра стейтов (их содержимого) так же добавлена новая команда  `genin list-state`
которая отобразит последние 10 запусков команды `genin upgrade`. В отображаемой информации
будет тип операции, аргументы с которыми вызывался `genin`, а так же список изменений (
...
Что бы произвести `upgrade` на основе `state` (то есть на основе предыдущего `upgrade`), можно:
...
Так же в некоторых случаях может быть удобно сохранять весь `geninstate` где то в одной директории.
Это можно сделать с помощью аргумента `--state-dir` или задав переменную окружения `GENIN_STATE_DIR`.

Аргумент `--export-state` может быть полезен для того что бы сохранить стейт апгрейда под каким-то
другим именем.
