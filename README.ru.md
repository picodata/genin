...
---
#### Использование `failure_domain` в качестве `zone` параметра инстансов

`Genin` предоставляет функционал `failure_domains`, описанный в предыдущем параграфе - в результате его работы, инстансы распределяются на нужные хосты, основываясь на их `failure_domains` конфигурации.

Есть возможность использовать финальный хост (конкретный `failure_domain`, куда распределяется инстанс) в качестве [`zone`](https://github.com/tarantool/ansible-cartridge/blob/master/doc/variables.md?plain=1#L90) параметра ansible-конфигурации инстанса. Для этого необходимо передать флаг `--fd-as-zone` в команду `build`, например: `genin build --fd-as-zone`.
...
1. Имя хоста, на который был распределен инстанс, используется в качестве параметра `zone` ansible-конфигурации инстанса.
