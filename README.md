...
```shell
curl -sLO https://binary.picodata.io/repository/raw/genin/bin/genin-0.9.0-x86_64-musl.tar.gz
tar -xvf genin-0.9.0-x86_64-musl.tar.gz ; install genin /usr/local/bin/
```
...
homebrew:
```shell
curl -L https://binary.picodata.io/repository/raw/genin/apple/genin-0.9.0-darwin-amd64.zip -o genin-0.9.0-darwin-amd64.zip
unzip genin-0.9.0-darwin-amd64.zip -d ~/bin/
```
